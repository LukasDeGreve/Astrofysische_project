...
struct node  { 
tuple<vector<double>, double> data; 
struct node *I; 
struct node *II;
struct node *III; 
struct node *IV; 
...
//allocates new node 
node* newNode(tuple<vector<double>, double> data) { 
...
node* Insert(node* root,Leaf data) {
	if(root == NULL) { // empty tree
		tuple<vector<double>, double> tup;
		tup = std::make_tuple(data.com(), data.width());
		root = newNode(tup);
	}
...
vector<Leaf> find(node* root, vector<double> deeltje) {
	if (root == NULL) {}
	else {
		//if (root->data.com())
	}
...
tuple<vector<double>, double> tup;
tup = std::make_tuple(hoofdruimte.com(), hoofdruimte.width());
