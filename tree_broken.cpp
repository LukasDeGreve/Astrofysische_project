...
struct node  { 
tuple<vector<double>, vector<double>, double> data; 
struct node *I; 
struct node *II;
struct node *III; 
struct node *IV; 
...
//allocates new node 
node* newNode(tuple<vector<double>, vector<double>, double> data) { 
...
node* Insert(node* root,Leaf data) {
	if(root == NULL) { // empty tree
		tuple<vector<double>, vector<double>, double> tup;
...
vector<double> force(node* root, vector<double> deeltje) {
	vector<double> kracht= {0, 0, 0};

	if (root == NULL) {kracht = {0, 0, 0};}
	else {
		double delta = 1;
		if (get<2>(root->data) > delta) {
			...
			vector<double> r = deeltje[:2] - get<0>(root->data);
			kracht = - (1 / r.norm3()) * r;
		}
	}
	return kracht;
...
while (inFile >> getal) {
	if (teller % 6 <= 2 && abs(getal) > num) { num = abs(getal) + 1;}
...
*/
for (Leaf kind : endlist) {
	cout << "x: " << kind.com()[0] << " y: " << kind.com()[1] << " z: " << kind.com()[2] << endl;
}
